...
| الإدخال | العلاقة |
|---------|---------|
| **LoRA** | **يبني عليه** — QLoRA = LoRA + NF4 quantized base |
| **LLM.int8()** | **يستخدم نفس المكتبة** — bitsandbytes |
| **Multi-LoRA Serving** | **يُمكّن** — محولات QLoRA أصغر = أكثر محولات على GPU |
| **Synthetic Data** | **تكاملي** — ولّد بيانات رخيصة + اضبط بـ QLoRA رخيص |

### القرار العملي: LoRA vs QLoRA (2026)

```
هل النموذج يتسع على GPU بـ FP16/BF16؟
  ├── نعم → LoRA العادي (جودة أعلى قليلاً)
  └── لا → QLoRA (الخيار الوحيد)

هل تحتاج أقصى جودة ممكنة؟
  ├── نعم → LoRA (FP16 base)
  └── لا → QLoRA (أرخص 3-5× مقابل فقدان 1-3% جودة)
```
