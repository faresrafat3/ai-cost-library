...
| الإدخال | العلاقة |
|---------|---------|
| **FP8** | **بديل أفضل** على Hopper+ — دقة أعلى + أسرع |
| **AWQ/GPTQ 4-bit** | **بديل أكثر ضغطاً** — ربع الذاكرة مقابل نصف |
| **SmoothQuant** | **يُكمل** — LLM.int8() للأوزان فقط، SmoothQuant W8A8 |
| **QLoRA** | **يعتمد عليه** — QLoRA = NF4 base (مبني على نفس المبدأ) |
| **bitsandbytes** | **المكتبة** — LLM.int8() مُطبق فيها |

### شجرة القرار — متى تختار أي تكميم 8-bit (2026)

```
هل لديك Hopper/Blackwell GPU؟
  ├── نعم → FP8 (الأفضل — دعم أصلي)
  └── لا → هل تحتاج W8A8 (أوزان + تفعيلات)؟
            ├── نعم → SmoothQuant
            └── لا → LLM.int8() (أبسط — سطر واحد)
```
